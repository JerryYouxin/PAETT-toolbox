...
def filter_regionenergy(data):
    ...
    for b in data.keys():
        ...
        sumenergy = 0
        for key in E_region.keys():
            sumenergy += E_region[key][2][209]

        holdrate = 0.2
        
        for key in E_region.keys():
            if E_region[key][2][209] < (holdrate * sumenergy):
                continue
            
            E_region_new[key] = (E_region[key][0],E_region[key][1],E_region[key][2],E_region[key][3])
            I_train += E_region_new[key][3]
            O_train += E_region_new[key][2]
            
        print("{0}: Removed {1} unsiginificant regions containing dirty data, Remain {2} regions.".format(b,len(E_region.keys())-len(E_region_new.keys()), len(E_region_new.keys())))
        res[b] = (I_train, O_train, E_region_new)
    return res
...
def test_result(modelretune,data):
    datasets = LOOCV_split_dataset(data)
    model = modelretune
    for b in datasets.keys():
        # model = MLP_model_init()
...
def fine_tune(path,data):
    ...
    model = load(path)
    model.set_params(model__max_iter=10)
    model.fit(I_train,O_train)
...
if __name__=="__main__":
    ...
    #NPB_data = read_data(["BT20", "CG", "EP", "IS","FT",  "MG","LU","UA","SP", "BT-MZ", "SP-MZ","backprop","cfd","heartwall","hotspot","hotspot3D","leukocyte","lud","lavaMD","nn","nw","particlefilter","pathfinder","srad","streamcluster20"])
    #NPB_data = read_data(["BT20", "CG",  "FT", "IS", "MG","LU","UA", "BT-MZ", "SP-MZ","heartwall","hotspot3D","leukocyte","lud","lavaMD","nn","nw","particlefilter","srad"])
    #NPB_data = read_data(["BT20", "CG", "EP","FT",  "IS", "MG","LU","UA","BT-MZ", "SP-MZ","cfd","heartwall","hotspot","leukocyte","lud","lavaMD","nn","nw","srad"])
    #NPB_data = read_data(["BT20", "CG", "EP", "FT", "IS", "MG","LU","UA", "SP","BT-MZ", "SP-MZ","backprop","cfd20","heartwall","hotspot","hotspot3D","leukocyte","lud","lavaMD","nn","nw","particlefilter","pathfinder","srad","streamcluster20","miniMD","mgo"])
    #NPB_data = read_data(["BT20", "CG", "EP", "FT", "IS", "MG","LU","UA", "SP","BT-MZ", "SP-MZ","heartwall","hotspot","hotspot3D","leukocyte","lavaMD","particlefilter","srad","streamcluster20","miniMD","QuEST","mgo"])
    #NPB_data = read_data(["BT","CG","EP","FT","IS","MG","BT-MZ","SP-MZ"])MLP.pkl,"QuEST","mgo"
    #NPB_data = read_data(["BT20", "CG", "EP", "FT", "IS", "MG","LU","UA", "SP","BT-MZ", "SP-MZ","heartwall","hotspot","hotspot3D","leukocyte","lavaMD","particlefilter","srad","streamcluster20"]),"IS""nw",
    #NPB_data = read_data(["BT20", "CG", "EP", "FT", "MG","LU","UA", "SP","BT-MZ", "SP-MZ","backprop","cfd20","heartwall","hotspot3D","leukocyte","lud","lavaMD","nw","particlefilter","pathfinder","srad","streamcluster20","miniMD","mgo"]),"mgo1","miniMD1"
    #NPB_data = read_data(["miniMD","mgo"])"particlefilter",
    NPB_data = read_data(["BT20", "CG", "EP", "FT", "IS", "MG","LU","UA", "SP","BT-MZ", "SP-MZ","backprop","cfd20","heartwall","hotspot","hotspot3D","leukocyte","lud","lavaMD","nn","nw","particlefilter","pathfinder","srad","miniMD","mgo"])
    #NPB_data = read_data(["miniMD","mgo"])
    
    f = open("finetunedata","w")
    ...
    print("filter unsig-energy region")
    NPB_data = filter_regionenergy(NPB_data)
    ''''''
    print("fine-tune:")
    model = fine_tune("finetune4.pkl",NPB_data)
    #model = fine_tune("ft6/ft3.pkl",NPB_data)
    #model = load("MLP.pkl")
    
    print("test_resule:")
    test_result(model,NPB_data)
    '''   
    print("INFO: Save model to ./fine_tuneMLP.pkl")
    save(model, "fine_tuneMLP")
    f.close()
    '''
