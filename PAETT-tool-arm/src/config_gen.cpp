...
int init_msr() {
    ...
    for (int i = 0; i < PAETT_getNCPU(); ++i) {
        snprintf(buff, 64, "/dev/cpu/%d/msr", i);
...
void measure_overhead(int n_iter) {
    ...
    for(int i=0; i < n_iter; ++i) {
        PAETT_modFreqAll( MAKE_CORE_VALUE_FROM_FREQ(config_metric.core_min/10.0),  0);
        PAETT_modFreqAll( MAKE_CORE_VALUE_FROM_FREQ(config_metric.core_max/10.0),  0);
        //PAETT_modFreqAll(2200000, 0);
        //PAETT_modFreqAll(800000, 0);
    }
...
uint64_t __measure_core_latency_once(uint64_t freq) {
    ...
    for(int i=0;i<ncpu;++i) {
        ...
        while(core<freq-100000 || core>freq+100000) {
            PAETT_getCoreFreq(i, &core);
        }
    }
    clock_gettime(CLOCK_REALTIME,&t1);
